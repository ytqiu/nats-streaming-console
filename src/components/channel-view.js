...
export default class ChannelView extends Component {
    ...
    componentWillUnmount() {
        ...
        let { focusedChannel: { name } = {} } = this.state
    ...
    focusChannel = async (channel) => {
        let { focusedChannel: { name: channelName } = {} } = this.state
        ...
        const opts = last_seq ? { startAtSequence: Math.max(last_seq - 30, 0) } : { startAtTimeDelta: 300000 }
        const messages = []
