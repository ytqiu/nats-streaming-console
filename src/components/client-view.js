...
import { subscribeToChannel, unSubscribeFromChannel } from '../api/web-socket'
...
export default class ClientView extends Component {
    ...
    componentWillUnmount() {
        ...
        let { focusedSubscription: { _channel } = {} } = this.state
        unSubscribeFromChannel(_channel)
    }
    ...
    getMonitorData = async () => {
        ...
        this.setState({
            ...
            clientMap,
            loading: false,
            focusedClient: this.state.focusedClient ? this.state.focusedClient : find(clientMap, () => true),
            focusedSubscription: undefined,
        })
    }
    ...
    focusSubscription = async (subscription) => {
        ...
        let { focusedSubscription: { _channel: _channelName } = {} } = this.state
        if (_channelName == _channel) {
            return
        }
        ...
        const opts = last_sent ? { startAtSequence: Math.max(last_sent - 30, 0) } : { startAtTimeDelta: 300000 }
        const messages = []
